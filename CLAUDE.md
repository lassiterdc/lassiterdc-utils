...
This project uses a conda environment named `dcl_utils`.

- **Running tools**: Use `conda run -n dcl_utils <command>` or activate the environment first with `conda activate dcl_utils`.
- **Copier updates**: When running `copier update` through `conda run`, pass `--defaults` since there is no interactive terminal: `conda run -n dcl_utils copier update --trust --skip-tasks --defaults`.
